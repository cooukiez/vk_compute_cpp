...
VCW_QueueFamilyIndices App::find_qf(VkPhysicalDevice loc_phy_dev) {
    ...
    for (const auto &qf: loc_qf_props) {
        if (qf.queueFlags & VK_QUEUE_COMPUTE_BIT && qf.timestampValidBits)
            loc_qf_indices.qf_comp = i;
...
bool App::is_phy_dev_suitable(VkPhysicalDevice loc_phy_dev) {
    ...
    return loc_qf_indices.is_complete() && exts_supported && features.samplerAnisotropy;
}
...
void App::create_dev() {
    ...
    std::set<uint32_t> q_families = {qf_indices.qf_comp.value()};
    ...
    vkGetDeviceQueue(dev, qf_indices.qf_comp.value(), 0, &q_comp);
}